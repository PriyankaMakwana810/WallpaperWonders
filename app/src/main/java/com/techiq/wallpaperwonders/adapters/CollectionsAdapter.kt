...
class CollectionsAdapter(
    ...
    override fun onBindViewHolder(viewHolder: RecyclerView.ViewHolder, position: Int) {
        if (viewHolder is MyViewHolder) {
            ...
            if ((dataSet[position] as Media).type == "Video") {
                viewHolder.mBinder.videoIndicator.visibility = View.VISIBLE
                previewURL = (dataSet[position] as Media).image.replace("_640", "_340")
            } else {
                viewHolder.mBinder.videoIndicator.visibility = View.GONE
                previewURL = (dataSet[position] as Media).src.large.replace("_640", "_340")
            }
